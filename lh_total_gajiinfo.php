...
//
// Table class for lh_total_gaji
//
class clh_total_gaji extends cTable {
	var $id_gaji;
	var $nama;
	var $status;
	var $tanggal;
	...
	function __construct() {
		...
		$this->TableVar = 'lh_total_gaji';
		$this->TableName = 'lh_total_gaji';
		$this->TableType = 'VIEW';
		$this->ExportAll = TRUE;
		...
		// id_gaji
		$this->id_gaji = new cField('lh_total_gaji', 'lh_total_gaji', 'x_id_gaji', 'id_gaji', '`id_gaji`', '`id_gaji`', 3, -1, FALSE, '`id_gaji`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->id_gaji->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id_gaji'] = &$this->id_gaji;

		// nama
		$this->nama = new cField('lh_total_gaji', 'lh_total_gaji', 'x_nama', 'nama', '`nama`', '`nama`', 200, -1, FALSE, '`nama`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['nama'] = &$this->nama;

		// status
		$this->status = new cField('lh_total_gaji', 'lh_total_gaji', 'x_status', 'status', '`status`', '`status`', 202, -1, FALSE, '`status`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['status'] = &$this->status;

		// tanggal
		$this->tanggal = new cField('lh_total_gaji', 'lh_total_gaji', 'x_tanggal', 'tanggal', '`tanggal`', 'DATE_FORMAT(`tanggal`, \'%d/%m/%Y %H:%i:%s\')', 135, 7, FALSE, '`tanggal`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->tanggal->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateDMY"));
		$this->fields['tanggal'] = &$this->tanggal;

		// gaji_pokok
		$this->gaji_pokok = new cField('lh_total_gaji', 'lh_total_gaji', 'x_gaji_pokok', 'gaji_pokok', '`gaji_pokok`', '`gaji_pokok`', 5, -1, FALSE, '`gaji_pokok`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->gaji_pokok->FldDefaultErrMsg = $Language->Phrase("IncorrectFloat");
		$this->fields['gaji_pokok'] = &$this->gaji_pokok;

		// lembur
		$this->lembur = new cField('lh_total_gaji', 'lh_total_gaji', 'x_lembur', 'lembur', '`lembur`', '`lembur`', 5, -1, FALSE, '`lembur`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->lembur->FldDefaultErrMsg = $Language->Phrase("IncorrectFloat");
		$this->fields['lembur'] = &$this->lembur;

		// tunjangan_proyek
		$this->tunjangan_proyek = new cField('lh_total_gaji', 'lh_total_gaji', 'x_tunjangan_proyek', 'tunjangan_proyek', '`tunjangan_proyek`', '`tunjangan_proyek`', 5, -1, FALSE, '`tunjangan_proyek`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->tunjangan_proyek->FldDefaultErrMsg = $Language->Phrase("IncorrectFloat");
		$this->fields['tunjangan_proyek'] = &$this->tunjangan_proyek;

		// Total Gaji
		$this->Total_Gaji = new cField('lh_total_gaji', 'lh_total_gaji', 'x_Total_Gaji', 'Total Gaji', '`Total Gaji`', '`Total Gaji`', 5, -1, FALSE, '`Total Gaji`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->Total_Gaji->FldDefaultErrMsg = $Language->Phrase("IncorrectFloat");
		$this->fields['Total Gaji'] = &$this->Total_Gaji;
	}
	...
	// View URL
	function GetViewUrl() {
		return $this->KeyUrl("lh_total_gajiview.php", $this->UrlParm());
	}

	// Add URL
	function GetAddUrl() {
		return "lh_total_gajiadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("lh_total_gajiedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("lh_total_gajiadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("lh_total_gajidelete.php", $this->UrlParm());
	}
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				if ($this->Total_Gaji->Exportable) $Doc->ExportCaption($this->Total_Gaji);
			} else {
				if ($this->id_gaji->Exportable) $Doc->ExportCaption($this->id_gaji);
				if ($this->nama->Exportable) $Doc->ExportCaption($this->nama);
				if ($this->status->Exportable) $Doc->ExportCaption($this->status);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					if ($this->Total_Gaji->Exportable) $Doc->ExportField($this->Total_Gaji);
				} else {
					if ($this->id_gaji->Exportable) $Doc->ExportField($this->id_gaji);
					if ($this->nama->Exportable) $Doc->ExportField($this->nama);
					if ($this->status->Exportable) $Doc->ExportField($this->status);
		...
		if ($Doc->Horizontal) {
			...
			$Doc->ExportAggregate($this->id_gaji, '');
			$Doc->ExportAggregate($this->nama, '');
			$Doc->ExportAggregate($this->status, '');
			$Doc->ExportAggregate($this->tanggal, '');
			$Doc->ExportAggregate($this->gaji_pokok, '');
			$Doc->ExportAggregate($this->lembur, '');
			$Doc->ExportAggregate($this->tunjangan_proyek, '');
			$Doc->ExportAggregate($this->Total_Gaji, 'TOTAL');
			$Doc->EndExportRow();
		}
