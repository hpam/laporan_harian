...
$lh_input_nota_add = NULL; // Initialize page object first

class clh_input_nota_add extends clh_input_nota {
	...
	// Page object name
	var $PageObjName = 'lh_input_nota_add';
	...
	function Page_Init() {
		...
		if (!$Security->CanAdd()) {
			...
			$this->Page_Terminate("lh_input_notalist.php");
		}
		...
		switch ($this->CurrentAction) {
			...
			case "C": // Copy an existing record
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("lh_input_notalist.php"); // No matching record, return to list
				}
				break;
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "lh_input_notaview.php")
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			if (strval($this->jumlah_pembelian->EditValue) <> "" && is_numeric($this->jumlah_pembelian->EditValue)) $this->jumlah_pembelian->EditValue = ew_FormatNumber($this->jumlah_pembelian->EditValue, -2, -1, -2, -1);
	...
	function ValidateForm() {
		...
		if (!ew_CheckNumber($this->jumlah_pembelian->FormValue)) {
...
// Create page object
if (!isset($lh_input_nota_add)) $lh_input_nota_add = new clh_input_nota_add();

// Page init
$lh_input_nota_add->Page_Init();

// Page main
$lh_input_nota_add->Page_Main();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var lh_input_nota_add = new ew_Page("lh_input_nota_add");
lh_input_nota_add.PageID = "add"; // Page ID
var EW_PAGE_ID = lh_input_nota_add.PageID; // For backward compatibility

// Form object
var flh_input_notaadd = new ew_Form("flh_input_notaadd");

// Validate form
flh_input_notaadd.Validate = function(fobj) {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		if (elm && !ew_HasValue(elm))
			return ew_OnError(this, elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($lh_input_nota->id_dana->FldCaption()) ?>");
		elm = fobj.elements["x" + infix + "_nomor_nota"];
		if (elm && !ew_HasValue(elm))
			return ew_OnError(this, elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($lh_input_nota->nomor_nota->FldCaption()) ?>");
		elm = fobj.elements["x" + infix + "_nama_toko"];
		if (elm && !ew_HasValue(elm))
			return ew_OnError(this, elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($lh_input_nota->nama_toko->FldCaption()) ?>");
		elm = fobj.elements["x" + infix + "_tanggal_nota"];
		if (elm && !ew_HasValue(elm))
			return ew_OnError(this, elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($lh_input_nota->tanggal_nota->FldCaption()) ?>");
		elm = fobj.elements["x" + infix + "_tanggal_nota"];
		if (elm && !ew_CheckEuroDate(elm.value))
			return ew_OnError(this, elm, "<?php echo ew_JsEncode2($lh_input_nota->tanggal_nota->FldErrMsg()) ?>");
		elm = fobj.elements["x" + infix + "_jumlah_pembelian"];
		if (elm && !ew_HasValue(elm))
			return ew_OnError(this, elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($lh_input_nota->jumlah_pembelian->FldCaption()) ?>");
		elm = fobj.elements["x" + infix + "_jumlah_pembelian"];
		if (elm && !ew_CheckNumber(elm.value))
			return ew_OnError(this, elm, "<?php echo ew_JsEncode2($lh_input_nota->jumlah_pembelian->FldErrMsg()) ?>");
		elm = fobj.elements["x" + infix + "_pesan"];
		if (elm && !ew_HasValue(elm))
			return ew_OnError(this, elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($lh_input_nota->pesan->FldCaption()) ?>");
...
flh_input_notaadd.Form_CustomValidate = 
...
flh_input_notaadd.ValidateRequired = true;
<?php } else { ?>
flh_input_notaadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
flh_input_notaadd.Lists["x_id_dana"] = {"LinkField":"x_id_dana","Ajax":null,"AutoFill":false,"DisplayFields":["x_periode_pembiayaan","x_jumlah_dana","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<p><span id="ewPageCaption" class="ewTitle ewTableTitle"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeVIEW") ?><?php echo $lh_input_nota->TableCaption() ?></span></p>
<p class="phpmaker"><a href="<?php echo $lh_input_nota->getReturnUrl() ?>" id="a_GoBack" class="ewLink"><?php echo $Language->Phrase("GoBack") ?></a></p>
<?php $lh_input_nota_add->ShowPageHeader(); ?>
<?php
$lh_input_nota_add->ShowMessage();
?>
<form name="flh_input_notaadd" id="flh_input_notaadd" class="ewForm" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return ewForms[this.id].Submit();">
<br>
<input type="hidden" name="t" value="lh_input_nota">
<input type="hidden" name="a_add" id="a_add" value="A">
<table cellspacing="0" class="ewGrid"><tr><td class="ewGridContent">
<div class="ewGridMiddlePanel">
<table id="tbl_lh_input_notaadd" class="ewTable">
<?php if ($lh_input_nota->id_dana->Visible) { // id_dana ?>
	<tr id="r_id_dana"<?php echo $lh_input_nota->RowAttributes() ?>>
		<td class="ewTableHeader"><span id="elh_lh_input_nota_id_dana"><table class="ewTableHeaderBtn"><tr><td><?php echo $lh_input_nota->id_dana->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></td></tr></table></span></td>
		<td<?php echo $lh_input_nota->id_dana->CellAttributes() ?>><span id="el_lh_input_nota_id_dana">
<select id="x_id_dana" name="x_id_dana"<?php echo $lh_input_nota->id_dana->EditAttributes() ?>>
<?php
if (is_array($lh_input_nota->id_dana->EditValue)) {
	$arwrk = $lh_input_nota->id_dana->EditValue;
	$rowswrk = count($arwrk);
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($lh_input_nota->id_dana->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " selected=\"selected\"" : "";
		if ($selwrk <> "") $emptywrk = FALSE;
?>
<option value="<?php echo ew_HtmlEncode($arwrk[$rowcntwrk][0]) ?>"<?php echo $selwrk ?>>
<?php echo $arwrk[$rowcntwrk][1] ?>
<?php if ($arwrk[$rowcntwrk][2] <> "") { ?>
<?php echo ew_ValueSeparator(1,$lh_input_nota->id_dana) ?><?php echo $arwrk[$rowcntwrk][2] ?>
<?php } ?>
</option>
<?php
	}
...
<script type="text/javascript">
flh_input_notaadd.Lists["x_id_dana"].Options = <?php echo (is_array($lh_input_nota->id_dana->EditValue)) ? ew_ArrayToJson($lh_input_nota->id_dana->EditValue, 1) : "[]" ?>;
</script>
</span><?php echo $lh_input_nota->id_dana->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($lh_input_nota->nomor_nota->Visible) { // nomor_nota ?>
	<tr id="r_nomor_nota"<?php echo $lh_input_nota->RowAttributes() ?>>
		<td class="ewTableHeader"><span id="elh_lh_input_nota_nomor_nota"><table class="ewTableHeaderBtn"><tr><td><?php echo $lh_input_nota->nomor_nota->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></td></tr></table></span></td>
		<td<?php echo $lh_input_nota->nomor_nota->CellAttributes() ?>><span id="el_lh_input_nota_nomor_nota">
<input type="text" name="x_nomor_nota" id="x_nomor_nota" size="30" maxlength="255" value="<?php echo $lh_input_nota->nomor_nota->EditValue ?>"<?php echo $lh_input_nota->nomor_nota->EditAttributes() ?>>
</span><?php echo $lh_input_nota->nomor_nota->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($lh_input_nota->nama_toko->Visible) { // nama_toko ?>
	<tr id="r_nama_toko"<?php echo $lh_input_nota->RowAttributes() ?>>
		<td class="ewTableHeader"><span id="elh_lh_input_nota_nama_toko"><table class="ewTableHeaderBtn"><tr><td><?php echo $lh_input_nota->nama_toko->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></td></tr></table></span></td>
		<td<?php echo $lh_input_nota->nama_toko->CellAttributes() ?>><span id="el_lh_input_nota_nama_toko">
<input type="text" name="x_nama_toko" id="x_nama_toko" size="30" maxlength="255" value="<?php echo $lh_input_nota->nama_toko->EditValue ?>"<?php echo $lh_input_nota->nama_toko->EditAttributes() ?>>
</span><?php echo $lh_input_nota->nama_toko->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($lh_input_nota->tanggal_nota->Visible) { // tanggal_nota ?>
	<tr id="r_tanggal_nota"<?php echo $lh_input_nota->RowAttributes() ?>>
		<td class="ewTableHeader"><span id="elh_lh_input_nota_tanggal_nota"><table class="ewTableHeaderBtn"><tr><td><?php echo $lh_input_nota->tanggal_nota->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></td></tr></table></span></td>
		<td<?php echo $lh_input_nota->tanggal_nota->CellAttributes() ?>><span id="el_lh_input_nota_tanggal_nota">
<input type="text" name="x_tanggal_nota" id="x_tanggal_nota" value="<?php echo $lh_input_nota->tanggal_nota->EditValue ?>"<?php echo $lh_input_nota->tanggal_nota->EditAttributes() ?>>
<?php if (!$lh_input_nota->tanggal_nota->ReadOnly && !$lh_input_nota->tanggal_nota->Disabled && @$lh_input_nota->tanggal_nota->EditAttrs["readonly"] == "" && @$lh_input_nota->tanggal_nota->EditAttrs["disabled"] == "") { ?>
&nbsp;<img src="phpimages/calendar.png" id="flh_input_notaadd$x_tanggal_nota$" name="flh_input_notaadd$x_tanggal_nota$" alt="<?php echo $Language->Phrase("PickDate") ?>" title="<?php echo $Language->Phrase("PickDate") ?>" class="ewCalendar" style="border: 0;">
<script type="text/javascript">
ew_CreateCalendar("flh_input_notaadd", "x_tanggal_nota", "%d/%m/%Y");
</script>
<?php } ?>
</span><?php echo $lh_input_nota->tanggal_nota->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($lh_input_nota->jumlah_pembelian->Visible) { // jumlah_pembelian ?>
	<tr id="r_jumlah_pembelian"<?php echo $lh_input_nota->RowAttributes() ?>>
		<td class="ewTableHeader"><span id="elh_lh_input_nota_jumlah_pembelian"><table class="ewTableHeaderBtn"><tr><td><?php echo $lh_input_nota->jumlah_pembelian->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></td></tr></table></span></td>
		<td<?php echo $lh_input_nota->jumlah_pembelian->CellAttributes() ?>><span id="el_lh_input_nota_jumlah_pembelian">
<input type="text" name="x_jumlah_pembelian" id="x_jumlah_pembelian" size="30" value="<?php echo $lh_input_nota->jumlah_pembelian->EditValue ?>"<?php echo $lh_input_nota->jumlah_pembelian->EditAttributes() ?>>
</span><?php echo $lh_input_nota->jumlah_pembelian->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($lh_input_nota->pesan->Visible) { // pesan ?>
	<tr id="r_pesan"<?php echo $lh_input_nota->RowAttributes() ?>>
		<td class="ewTableHeader"><span id="elh_lh_input_nota_pesan"><table class="ewTableHeaderBtn"><tr><td><?php echo $lh_input_nota->pesan->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></td></tr></table></span></td>
		<td<?php echo $lh_input_nota->pesan->CellAttributes() ?>><span id="el_lh_input_nota_pesan">
<textarea name="x_pesan" id="x_pesan" cols="25" rows="4"<?php echo $lh_input_nota->pesan->EditAttributes() ?>><?php echo $lh_input_nota->pesan->EditValue ?></textarea>
<script type="text/javascript">
ew_CreateEditor("flh_input_notaadd", "x_pesan", 25, 4, <?php echo ($lh_input_nota->pesan->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span><?php echo $lh_input_nota->pesan->CustomMsg ?></td>
	</tr>
<?php } ?>
</table>
</div>
...
<script type="text/javascript">
flh_input_notaadd.Init();
</script>
<?php
$lh_input_nota_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$lh_input_nota_add->Page_Terminate();
?>
