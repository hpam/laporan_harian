...
//
// Table class for nota harian
//
class cnota_harian extends cTable {
	var $id_nota;
	var $id_dana;
	var $nomor_nota;
	var $nama_toko;
	...
	function __construct() {
		...
		$this->TableVar = 'nota_harian';
		$this->TableName = 'nota harian';
		$this->TableType = 'VIEW';
		$this->ExportAll = TRUE;
		...
		// id_nota
		$this->id_nota = new cField('nota_harian', 'nota harian', 'x_id_nota', 'id_nota', '`id_nota`', '`id_nota`', 3, -1, FALSE, '`id_nota`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->id_nota->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id_nota'] = &$this->id_nota;

		// id_dana
		$this->id_dana = new cField('nota_harian', 'nota harian', 'x_id_dana', 'id_dana', '`id_dana`', '`id_dana`', 3, 7, FALSE, '`id_dana`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->id_dana->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id_dana'] = &$this->id_dana;

		// nomor_nota
		$this->nomor_nota = new cField('nota_harian', 'nota harian', 'x_nomor_nota', 'nomor_nota', '`nomor_nota`', '`nomor_nota`', 200, -1, FALSE, '`nomor_nota`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['nomor_nota'] = &$this->nomor_nota;

		// nama_toko
		$this->nama_toko = new cField('nota_harian', 'nota harian', 'x_nama_toko', 'nama_toko', '`nama_toko`', '`nama_toko`', 200, -1, FALSE, '`nama_toko`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['nama_toko'] = &$this->nama_toko;

		// tanggal_nota
		$this->tanggal_nota = new cField('nota_harian', 'nota harian', 'x_tanggal_nota', 'tanggal_nota', '`tanggal_nota`', 'DATE_FORMAT(`tanggal_nota`, \'%m/%d/%Y %H:%i:%s\')', 133, 7, FALSE, '`tanggal_nota`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		...
		// jumlah_pembelian
		$this->jumlah_pembelian = new cField('nota_harian', 'nota harian', 'x_jumlah_pembelian', 'jumlah_pembelian', '`jumlah_pembelian`', '`jumlah_pembelian`', 3, -1, FALSE, '`jumlah_pembelian`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		...
		// pesan
		$this->pesan = new cField('nota_harian', 'nota harian', 'x_pesan', 'pesan', '`pesan`', '`pesan`', 201, -1, FALSE, '`pesan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['pesan'] = &$this->pesan;

		// jumlah_dana
		$this->jumlah_dana = new cField('nota_harian', 'nota harian', 'x_jumlah_dana', 'jumlah_dana', '`jumlah_dana`', '`jumlah_dana`', 3, -1, FALSE, '`jumlah_dana`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->jumlah_dana->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['jumlah_dana'] = &$this->jumlah_dana;
	}
	...
	// View URL
	function GetViewUrl() {
		return $this->KeyUrl("nota_harianview.php", $this->UrlParm());
	}

	// Add URL
	function GetAddUrl() {
		return "nota_harianadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("nota_harianedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("nota_harianadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("nota_hariandelete.php", $this->UrlParm());
	}
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				if ($this->jumlah_dana->Exportable) $Doc->ExportCaption($this->jumlah_dana);
			} else {
				if ($this->id_nota->Exportable) $Doc->ExportCaption($this->id_nota);
				if ($this->id_dana->Exportable) $Doc->ExportCaption($this->id_dana);
				if ($this->nomor_nota->Exportable) $Doc->ExportCaption($this->nomor_nota);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					if ($this->jumlah_dana->Exportable) $Doc->ExportField($this->jumlah_dana);
				} else {
					if ($this->id_nota->Exportable) $Doc->ExportField($this->id_nota);
					if ($this->id_dana->Exportable) $Doc->ExportField($this->id_dana);
					if ($this->nomor_nota->Exportable) $Doc->ExportField($this->nomor_nota);
		...
		if ($Doc->Horizontal) {
			...
			$Doc->ExportAggregate($this->jumlah_dana, 'AVERAGE');
			$Doc->EndExportRow();
		}
