...
function CurrentProjectID() {
	...
	return "{67264FB2-6364-478B-87DD-B3E0D7A29425}";
}
...
class cAdvancedSecurity {
			...
			if ($rs = $conn->Execute($sSql)) {
				if (!$rs->EOF) {
					...
					if ($ValidateUser) {
						...
						if (is_null($rs->fields('id_level'))) {
							$this->setSessionUserLevelID(0);
						} else {
							$this->setSessionUserLevelID(intval($rs->fields('id_level'))); // Load User Level
						}
						$this->SetUpUserLevel();
	...
	// function to check if user level table exist
