...
$view1_add = NULL; // Initialize page object first

class cview1_add extends cview1 {
		...
		switch ($this->CurrentAction) {
			...
			case "C": // Copy an existing record
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("view1list.php"); // No matching record, return to list
				}
				break;
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "view1view.php")
	...
	function ValidateForm() {
		...
		if (!ew_CheckInteger($this->jumlah_dana->FormValue)) {
...
// Create page object
if (!isset($view1_add)) $view1_add = new cview1_add();
...
// Page object
var view1_add = new ew_Page("view1_add");
view1_add.PageID = "add"; // Page ID
var EW_PAGE_ID = view1_add.PageID; // For backward compatibility

// Form object
var fview1add = new ew_Form("fview1add");

// Validate form
fview1add.Validate = function(fobj) {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		if (elm && !ew_HasValue(elm))
			return ew_OnError(this, elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($view1->id_dana->FldCaption()) ?>");
		elm = fobj.elements["x" + infix + "_nomor_nota"];
		if (elm && !ew_HasValue(elm))
			return ew_OnError(this, elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($view1->nomor_nota->FldCaption()) ?>");
		elm = fobj.elements["x" + infix + "_nama_toko"];
		if (elm && !ew_HasValue(elm))
			return ew_OnError(this, elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($view1->nama_toko->FldCaption()) ?>");
		elm = fobj.elements["x" + infix + "_tanggal_nota"];
		if (elm && !ew_HasValue(elm))
			return ew_OnError(this, elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($view1->tanggal_nota->FldCaption()) ?>");
		elm = fobj.elements["x" + infix + "_tanggal_nota"];
		...
		if (elm && !ew_HasValue(elm))
			return ew_OnError(this, elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($view1->jumlah_pembelian->FldCaption()) ?>");
		elm = fobj.elements["x" + infix + "_jumlah_pembelian"];
		if (elm && !ew_CheckInteger(elm.value))
			return ew_OnError(this, elm, "<?php echo ew_JsEncode2($view1->jumlah_pembelian->FldErrMsg()) ?>");
		elm = fobj.elements["x" + infix + "_pesan"];
		if (elm && !ew_HasValue(elm))
			return ew_OnError(this, elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($view1->pesan->FldCaption()) ?>");
		elm = fobj.elements["x" + infix + "_jumlah_dana"];
		if (elm && !ew_HasValue(elm))
			return ew_OnError(this, elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($view1->jumlah_dana->FldCaption()) ?>");
		elm = fobj.elements["x" + infix + "_jumlah_dana"];
		if (elm && !ew_CheckInteger(elm.value))
...
fview1add.ValidateRequired = true;
<?php } else { ?>
fview1add.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fview1add.Lists["x_id_dana"] = {"LinkField":"x_id_dana","Ajax":null,"AutoFill":false,"DisplayFields":["x_periode_pembiayaan","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<p><span id="ewPageCaption" class="ewTitle ewTableTitle"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeVIEW") ?><?php echo $view1->TableCaption() ?></span></p>
<p class="phpmaker"><a href="<?php echo $view1->getReturnUrl() ?>" id="a_GoBack" class="ewLink"><?php echo $Language->Phrase("GoBack") ?></a></p>
<?php $view1_add->ShowPageHeader(); ?>
<?php
$view1_add->ShowMessage();
?>
<form name="fview1add" id="fview1add" class="ewForm" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return ewForms[this.id].Submit();">
<br>
<input type="hidden" name="t" value="view1">
<input type="hidden" name="a_add" id="a_add" value="A">
<table cellspacing="0" class="ewGrid"><tr><td class="ewGridContent">
<div class="ewGridMiddlePanel">
<table id="tbl_view1add" class="ewTable">
<?php if ($view1->id_dana->Visible) { // id_dana ?>
	<tr id="r_id_dana"<?php echo $view1->RowAttributes() ?>>
		<td class="ewTableHeader"><span id="elh_view1_id_dana"><table class="ewTableHeaderBtn"><tr><td><?php echo $view1->id_dana->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></td></tr></table></span></td>
		<td<?php echo $view1->id_dana->CellAttributes() ?>><span id="el_view1_id_dana">
...
if (is_array($view1->id_dana->EditValue)) {
	...
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($view1->id_dana->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " selected=\"selected\"" : "";
...
<script type="text/javascript">
fview1add.Lists["x_id_dana"].Options = <?php echo (is_array($view1->id_dana->EditValue)) ? ew_ArrayToJson($view1->id_dana->EditValue, 1) : "[]" ?>;
</script>
</span><?php echo $view1->id_dana->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($view1->nomor_nota->Visible) { // nomor_nota ?>
	<tr id="r_nomor_nota"<?php echo $view1->RowAttributes() ?>>
		<td class="ewTableHeader"><span id="elh_view1_nomor_nota"><table class="ewTableHeaderBtn"><tr><td><?php echo $view1->nomor_nota->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></td></tr></table></span></td>
		<td<?php echo $view1->nomor_nota->CellAttributes() ?>><span id="el_view1_nomor_nota">
...
</span><?php echo $view1->nomor_nota->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($view1->nama_toko->Visible) { // nama_toko ?>
	<tr id="r_nama_toko"<?php echo $view1->RowAttributes() ?>>
		<td class="ewTableHeader"><span id="elh_view1_nama_toko"><table class="ewTableHeaderBtn"><tr><td><?php echo $view1->nama_toko->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></td></tr></table></span></td>
		<td<?php echo $view1->nama_toko->CellAttributes() ?>><span id="el_view1_nama_toko">
...
</span><?php echo $view1->nama_toko->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($view1->tanggal_nota->Visible) { // tanggal_nota ?>
	<tr id="r_tanggal_nota"<?php echo $view1->RowAttributes() ?>>
		<td class="ewTableHeader"><span id="elh_view1_tanggal_nota"><table class="ewTableHeaderBtn"><tr><td><?php echo $view1->tanggal_nota->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></td></tr></table></span></td>
		<td<?php echo $view1->tanggal_nota->CellAttributes() ?>><span id="el_view1_tanggal_nota">
...
&nbsp;<img src="phpimages/calendar.png" id="fview1add$x_tanggal_nota$" name="fview1add$x_tanggal_nota$" alt="<?php echo $Language->Phrase("PickDate") ?>" title="<?php echo $Language->Phrase("PickDate") ?>" class="ewCalendar" style="border: 0;">
<script type="text/javascript">
ew_CreateCalendar("fview1add", "x_tanggal_nota", "%d/%m/%Y");
</script>
<?php } ?>
</span><?php echo $view1->tanggal_nota->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($view1->jumlah_pembelian->Visible) { // jumlah_pembelian ?>
	<tr id="r_jumlah_pembelian"<?php echo $view1->RowAttributes() ?>>
		<td class="ewTableHeader"><span id="elh_view1_jumlah_pembelian"><table class="ewTableHeaderBtn"><tr><td><?php echo $view1->jumlah_pembelian->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></td></tr></table></span></td>
		<td<?php echo $view1->jumlah_pembelian->CellAttributes() ?>><span id="el_view1_jumlah_pembelian">
...
</span><?php echo $view1->jumlah_pembelian->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($view1->pesan->Visible) { // pesan ?>
	<tr id="r_pesan"<?php echo $view1->RowAttributes() ?>>
		<td class="ewTableHeader"><span id="elh_view1_pesan"><table class="ewTableHeaderBtn"><tr><td><?php echo $view1->pesan->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></td></tr></table></span></td>
		<td<?php echo $view1->pesan->CellAttributes() ?>><span id="el_view1_pesan">
...
<script type="text/javascript">
ew_CreateEditor("fview1add", "x_pesan", 35, 4, <?php echo ($view1->pesan->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span><?php echo $view1->pesan->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($view1->jumlah_dana->Visible) { // jumlah_dana ?>
	<tr id="r_jumlah_dana"<?php echo $view1->RowAttributes() ?>>
		<td class="ewTableHeader"><span id="elh_view1_jumlah_dana"><table class="ewTableHeaderBtn"><tr><td><?php echo $view1->jumlah_dana->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></td></tr></table></span></td>
		<td<?php echo $view1->jumlah_dana->CellAttributes() ?>><span id="el_view1_jumlah_dana">
...
</span><?php echo $view1->jumlah_dana->CustomMsg ?></td>
	</tr>
<?php } ?>
</table>
</div>
